!function (a, b) {
    ...
    !function (a) {
        ...
        "use strict";
        b.module("LogicifyGMap").directive("logicifyGmapDraw", ["$timeout", "$log", "$q", "$compile", "$http", function (b, c, d, e, f) {
            return {
                restrict: "E", require: "^logicifyGmap", scope: {}, link: function (b, c, d, e) {
                    var f = e.getMap(), g = new a.maps.drawing.DrawingManager({
                        drawingMode: a.maps.drawing.OverlayType.MARKER,
                        drawingControl: !0,
                        drawingControlOptions: {
                            position: a.maps.ControlPosition.TOP_CENTER,
                            drawingModes: [a.maps.drawing.OverlayType.MARKER, a.maps.drawing.OverlayType.CIRCLE, a.maps.drawing.OverlayType.POLYGON, a.maps.drawing.OverlayType.POLYLINE, a.maps.drawing.OverlayType.RECTANGLE]
                        },
                        ...
                        polygonOptions: {editable: !0, fillColor: "#ffffff"},
                        rectangleOptions: {fillColor: "#ffffff"}
                    });
                    g.setMap(f), a.maps.event.addListener(drawingManager, "overlaycomplete", function (a) {
                        a.overlay.set("editable", !0)
                    })
                }
            }
        }])
        ...
        b.module("LogicifyGMap").directive("logicifyGmapControl", ["$compile", "$log", "$timeout", function (c, d, e) {
            return {
                ...
                scope: {controlPosition: "&controlPosition", controlIndex: "&controlIndex", events: "&events"},
                link: function (d, f, g, h) {
                    ...
                    var j = d.controlPosition(), k = d.controlIndex(), l = d.events(), m = b.element(f.html().trim()), n = [], o = [];
                    c(m)(d), e(function () {
                        d.$apply()
                    ...
                    var p = h.getMap();
                    if (!p.controls[j])throw new Error("Position of control on the map is invalid. Please see google maps spec.");
                    p.controls[j].push(m[0]), null != l && b.forEach(l, function (a, b) {
                        "function" == typeof a && ("fileSelect" === b ? m[0]instanceof HTMLInputElement && "file" === m[0].type ? o.push(m.bind("change", function () {
                            a(this.files[0])
                        })) : o.push(m.find("input:file").bind("change", function () {
                            a(this.files[0])
                        })) : n.push(i(b, a)))
                    })
                }
                ...
                link: function (f, i, j, k) {
                    ...
                    function m() {
                        return f.$watch("kmlCollection._uid", function (a, b) {
                            (null == a || a != z) && (f.kmlCollection instanceof h || (f.kmlCollection = new h(f.kmlCollection)), z = f.kmlCollection._uid, (f.busy === !0 || y.docs && y.docs.length > 0) && (A.forEach(function (a) {
                                a._abort()
                    ...
                    function n(a) {
                        null != a && w(a).then(function (a) {
                            1 != f.kmlCollection.length && f.fitBoundsAfterAll !== !1 && r()
                        })
                    }
                    ...
                    function r() {
                        f.globalBounds = new a.maps.LatLngBounds, 1 != f.kmlCollection.length && f.fitBoundsAfterAll !== !1 ? (f.kmlCollection.forEach(function (a) {
                            a.doc && f.globalBounds.extend(a.doc[0].bounds.getCenter())
                        }), c(function () {
                    ...
                    if (!geoXML3)throw new Error("You should include geoxml3.js to be able to parse xml overlays. Please check that geoxml3.js file loads before logicify-gmap.js");
                    var y = null;
                    f.kmlCollection = new h(f.kmlCollection);
                    var z = f.kmlCollection._uid;
                    f.events = f.gmapEvents() || {}, f.parserOptions = f.parserOptions() || {}, f.onProgress = f.onProgress(), f.fitBoundsAfterAll = f.fitAllLayers(), f.infowindow = f.infoWindow;
                    var A = [], B = {PENDING: 0, RESOLVED: 1, REJECTED: 2};
                    f.gMap = k.getMap(), f.collectionsWatcher = m(), f.infowindow && "function" == typeof f.infowindow.$ready ? f.infowindow.$ready(function (a) {
                        y = new geoXML3.parser(l(f.gMap, a)), t()
                    }) : (y = new geoXML3.parser(l(f.gMap)), t()), f.$on("$destroy", function () {
